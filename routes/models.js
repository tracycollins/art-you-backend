...
router.param("id", async (req, res, next, id) => {
  ...
  try {
    const doc = await global.art47db[model].findOne({ id: id });
...
router.get("/:id", async (req, res) => {
  ...
  const docs = await global.art47db[model].find(query).lean();
...
router.post("/:id", async (req, res, next) => {
  ...
  try {
    const doc = new global.art47db[model](req.body);
    await doc.save();
    res.json(doc);
  } catch (err) {
    ...
    res
      .status(400)
      .send(`POST | ${model} | ID: ${req.body.id} | ERROR: ${err}`);
  }
});
