...
router.get('/:id', async (req, res) => {
  ...
  const doc = await global.artyouDb[model].findOne(query).populate('image').populate({path: 'artist', populate: { path: 'image'}}).populate('tags').lean();
...
router.get('/', async (req, res) => {
  try{
    ...
    const docs = await global.artyouDb[model].find({}).populate('image').populate({path: 'artist', populate: { path: 'image'}}).populate('tags').lean();
