  ...
  try {
    ...
    const processFunction = async (artwork, i, user_id, results, next) => {
      ...
      if (!user_id) {
        ...
        console.log(
          `... NO USER | ARTWORK _ID: ${artwork._id} | INDEX: ${i} | RATINGS: ${artwork.ratings.length} | ${results.length} RESULTS`
        );
      } else {
      ...
      if (i % 100 === 0) {
        console.log(
          `... USER: ${user_id} | ARTWORK _ID: ${artwork._id} | INDEX: ${i} | RATINGS: ${artwork.ratings.length} | ${results.length} RESULTS`
        );
      }
    };
