  ...
  if (
    ...
    console.log(
      `DB | SEED | processImagefile | transformImage` +
        ` | IMAGE: ${transformImageResults.size} BYTES / ${transformImageResults.width}w X ${transformImageResults.height}h` +
        ` | FILE: ${artworkImageSmallPath}`
    );
    const keyNameSmall = `${artistDoc.userName}/images/${imageFileSmall}`;
    ...
    if (
      ...
      (!artworkDoc.imageAnalysis || FORCE_IMAGE_ANALYSIS)
    ) {
      console.log(
      ...
      try {
        ...
        if (err.code && err.code === 8) {
          console.log(`DB | SEED | ANALYZE IMAGE ERROR | QUOTA EXHAUSTED`);
  ...
  try {
    ...
    if (enableRandomImages) {
      ...
      for (let i = maxImages; i > 0; i--) {
        ...
        imageFilePaths.push(randomImageFilePath);
      }

      // imageFilePaths = _.sampleSize(
