...
const updateDbModel = async (params) => {
  ...
  try {
    ...
    // );
    let dbDoc = await global.artyouDb[modelObj.type]
      .findOne({ id: modelObj.id })
      .lean();
  ...
  try {
    ...
    for (const imageFile of imageFiles) {
      if (
        ...
        const artworkImagePath = path.join("/", params.imageFolder, imageFile);
        const artworkImageAnalysisPath = path.join(
          "/",
          params.analysisFolder,
          imageAnalysisFile
        );
        ...
        if (enableRandomImages) {
          ...
          jsonfile.writeFileSync(
            artworkImageAnalysisPath,
            fakeImageAnalysisObj
          );
        }
        ...
        if (fs.existsSync(artworkImageAnalysisPath)) {
          console.log(
            `DB | SEED | s3putImage | ARTIST: ${params.artistDoc.displayName} | TITLE: ${imageTitle} | IMAGE ANALYSIS: ${artworkImageAnalysisPath}`
          );

          const keyName = `${params.artistDoc.userName}/image_analysis/${imageAnalysisFile}`;
        ...
        console.log(
          `DB | SEED | s3putImage | ARTIST: ${params.artistDoc.displayName} | TITLE: ${imageTitle} | IMAGE: ${artworkImagePath}`
        );
      } else {
...
const seedDb = async (p) => {
  try {
    const params = p || {};
    let results = {};
    const enableLoadArtists = params.enableLoadArtists || false;
    const enableFake = params.enableFake || false;
...
const main = async () => {
  try {
    ...
    results = await seedDb({ enableLoadArtists: true, enableFake: false });
