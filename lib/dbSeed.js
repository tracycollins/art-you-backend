...
const DEFAULT_IMAGE_FILE_USED_FILE_PATH =
  "/Volumes/RAID1/projects/art47-backend/imageFileUsed.json";

let googleVisionQuotaExhausted = false;

const DEFAULT_GOOGLE_STORAGE_BUCKET = "art47";

const MAX_GOOGLE_FAKE_IMAGE_ANALYSIS_TAGS = 20;
...
const DEFAULT_FAKE_ARTISTS_FOLDER =
  "/Users/tc/Dropbox/Apps/art47-fake/artwork/artists";
...
const defaultSeedRootDir =
  process.env.ART47_SEED_ROOT_DIR || "/Users/tc/Dropbox/Apps/art47";
...
const defaultFakeSeedRootDir =
  process.env.ART47_FAKE_SEED_ROOT_DIR || "/Users/tc/Dropbox/Apps/art47-fake";
...
const defaultArtistsRootUrl =
  process.env.ART47_ARTISTS_ROOT_URL ||
  "https://art47-artists.s3.amazonaws.com";
const defaultArtworksRootUrl =
  process.env.ART47_ARTWORKS_ROOT_URL ||
  "https://art47-artworks.s3.amazonaws.com";
...
const getRandomFilePath = async (params) => {
  ...
  const usedFiles = Object.keys(imageFileUsedHashMap);
  const unusedFiles = files.filter((file) => !usedFiles.includes(file));
  const randomFile = _.sample(unusedFiles);

  if (unusedFiles.length === 0) {
    console.log(
      chalk.yellow(
        `DB | SEED | !!! NO UNUSED IMAGE FILES: ${files.length}` +
  ...
  console.log(
    `DB | SEED | IMAGE FILES: ${files.length}` +
      ` | USED: ${usedFiles.length}` +
      ` | UNUSED: ${unusedFiles.length}` +
      ` | PATH: ${params.folder}` +
      ` | RANDOM: ${randomFile}`
  ...
  imageFileUsedHashMap[randomFile] =
    params.meta !== undefined ? params.meta : getTimeStamp(0);
...
const updateDbModel = async (params) => {
  ...
  try {
    let dbDoc = await global.art47db[modelObj.type].findOne({
      id: modelObj.id,
    });
    ...
    if (dbDoc) {
      ...
      dbDoc = new global.art47db[modelObj.type](modelObj);
  ...
  try {
    ...
    switch (model) {
      case "Artist":
        ...
        const artworksLoaded = await loadArtworks({
  ...
  fileQueueInterval = setInterval(async () => {
    if (fileQueueReady && stats.walk.complete && fileQueue.length === 0) {
      ...
      try {
        ...
        console.log(chalkError(`DB | processArtistArtworkFile ERROR: ${err}`));
        fileQueueReady = true;
      }
    }
  }, params.interval);
...
const processArtistArtworkFile = async (params) => {
  try {
    ...
    if (artist && path.parse(file).base === defaultArtistInfoFileName) {
      ...
      console.log(
        chalk.blue(
          `DB | SEED | +++ ARTIST [${stats.artists.processed}]` +
            ` _ID: ${artistDoc._id}` +
            ` | ID: ${artistDoc.id}` +
...
const updateArtworkTagsFromImageAnalysis = async (p) => {
  try {
    ...
    //art47/artwork/artists/@21shield/image_analysis/google/nutella_jarson_00001/output-1-to-1.json
    // Downloads the file
            ...
            for (const imagePropertyAnnotationKey of Object.keys(
              ...
              if (imagePropertyAnnotationKey === "dominantColors") {
                for (const color of response.imagePropertiesAnnotation
                  ...
                  const dominantColorTag =
                    colorName.pantone[0].name.toLowerCase();
    ...
    for (const tagName of tagNameSet) {
      ...
      let tagDoc = await global.art47db.Tag.findOne({
        id: tagName.toLowerCase(),
      });

      if (!tagDoc) {
        tagDoc = new global.art47db.Tag({ id: tagName.toLowerCase() });
...
const loadArtworks = async (p) => {
  try {
    ...
    const bucketName = params.bucketName || "art47-artworks";
    ...
    if (enableRandomImages) {
      ...
      for (let i = maxImages; i > 0; i--) {
        ...
        if (!randomImageFilePath) {
          console.log(
            chalk.red(`DB | SEED | loadArtworks | !!! NO UNUSED IMAGE FILES`)
          );
          break;
        }
