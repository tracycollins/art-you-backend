...
const ENABLE_FAKE = false;
const ENABLE_LOAD_ARTISTS = true;
const FORCE_IMAGE_ANALYSIS = true;
...
const updateArtworkTagsFromImageAnalysis = async (p) => {
  ...
  try {
    ...
    const imageAnalysisPath = params.imageAnalysisPath;
    const type = params.type || "google";

    // const imageAnalysisObj = await jsonfile.readFile(imageAnalysisPath);
    const imageAnalysisObj = await jsonfile.readFile(imageAnalysisPath);

    const tagUpdateArray = [];

    switch (type) {
      default:
        for (const response of imageAnalysisObj.responses) {
          for (const analysisType of Object.keys(response)) {
            for (const element of response[analysisType]) {
              const tagDoc = await global.artyouDb.Tag.findOne({
                id: element.description.toLowerCase(),
              });
              if (tagDoc) {
                tagUpdateArray.push(tagDoc);
              }
            }
          }
        }
    ...
    artworkDoc.tags = tagUpdateArray;
    return artworkDoc;
  } catch (err) {
    console.error(
...
const processImagefile = async (p) => {
  ...
  if (
    ...
    if (enableRandomImages) {
      await fs.copy(
        path.join("/", randomImageDir, imageFile),
        artworkImagePath
      );
    ...
    if (!artworkDoc.imageAnalysis || FORCE_IMAGE_ANALYSIS) {
      // if (!fs.existsSync(artworkImageAnalysisPath) || FORCE_IMAGE_ANALYSIS) {
      console.log(
      ...
      try {
        const imageAnalysisResults = await imageTools.analyzeImage({
          artist: artistDoc.id,
          imageFile: imageFile,
          destinationFilename: artworkImageAnalysisPath,
        });
        ...
        //   "imageFile": "DSCF2788.jpg",
        //   "options": {},
        //   "imageTechnology": "google",
        //   "analysis": {
        //     "outputConfig": {
        //       "gcsDestination": {
        //         "uri": "gs://art-you/artwork/artists/@dancarew/image_analysis/google/DSCF2788/"
        //       },
        //       "batchSize": 1
        //     }
        //   }
        ...
        //             },
        //             confidence: 0.65625,
        //             importanceFraction: 0.9696971,
        //           },
        //         ],
        //       },
        //       context: {
        //         uri:
        //           "gs://art-you/artwork/artists/@katequarfordt/images/Relationscapes.jpg",
        //       },
        //     },
        //   ],
        // };

        // if (!fs.existsSync(path.parse(artworkImageAnalysisPath).dir)) {
        //   mkdirp.sync(path.parse(artworkImageAnalysisPath).dir);
        // }
        // jsonfile.writeFileSync(artworkImageAnalysisPath, imageAnalysisResults);

        // console.log(
        //   `DB | SEED | +++ SAVED IMAGE ANALYSIS | ARTIST: ${artistDoc.displayName} | FILE: ${artworkImageAnalysisPath}`
        // );
      } catch (err) {
      ...
      // const keyName = `${artistDoc.userName}/image_analysis/${imageAnalysisFile}`;

      // await s3putImage({
      //   bucketName: bucketName,
      //   keyName: keyName,
      //   file: artworkImageAnalysisPath,
      // });

      artworkDoc = await updateArtworkTagsFromImageAnalysis({
        artworkDoc: artworkDoc,
        // imageAnalysisPath: artworkImageAnalysisPath,
      });
    }
