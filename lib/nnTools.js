...
const DEFAULT_NETWORK_FIT_EPOCHS = 5000;
  ...
  try {
    ...
    await cursor.eachAsync(async function (artworkDoc) {
      if (!artworkDoc) {
        ...
        console.log(
          chalk.green(`NNT | ###  UPDATE RECOMMENDATIONS COMPLETE  ###`)
        );
        console.log(
          chalk.green(
            `NNT | NN ID: ${networkId} | USERNAME: ${userDoc.userName}| OAUTH ID: ${userOauthID}`
          )
        );
        console.log(
      ...
      const rec = await global.artyouDb.Recommendation.findOneAndUpdate(
        {
          user: userDoc,
          artwork: artworkDoc,
        },
      ...
      artworkDoc.recommendations.addToSet(rec._id);
      await artworkDoc.save();
      numberArtworksUpdated += 1;
      console.log(
        chalkLog(
          `${MODULE_ID_PREFIX} | UPDATED REC | USER ID: ${
            rec.user.id
          } | ARTWORKS UPDATED: ${numberArtworksUpdated} | ARTWORK ID: ${
            rec.artwork.id
