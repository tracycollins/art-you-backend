...
NeuralNetworkTools.prototype.runNetworkTest = async function (p) {
  ...
  try{
    ...
    console.log(`NNT | SAVE DB INPUTS DOC | ${testInputsDoc.inputsId}`)
    // console.log(testInputsDoc)

    await testInputsDoc.save();
    console.log(`NNT | LOAD DB INPUTS DOC | ${testInputsDoc.inputsId}`)

    const loadedInputsDoc = await global.artyouDb.NetworkInputs.findOne({inputsId: testInputsDoc.inputsId}).lean()

    console.log(`NNT | CREATE NN DOC | INPUTS: ${loadedInputsDoc.inputsId}`)
    ...
    console.log(chalk.green(`NNT | FIT COMPLETE | NN ID: ${testNetworkDoc.networkId} | EPOCHS ${results.stats.params.epochs}`))
    console.log(chalk.green(`============================================================================================`))
