  ...
  try {
    ...
    if (networkId) {
      ...
      if (moment().valueOf() - moment(userDoc.network.createdAt) < MAX_NN_AGE) {
        console.log(
          chalk.blue(
            `NNT | USING USER NN | USER OAUTH ID: ${userOauthID} | AGE LESS THAN MAX: ${msToTime(
              MAX_NN_AGE
            )}`
          )
        );
        ...
        console.log(
          chalkAlert(
            `NNT | NN TOO OLD ... CREATING NEW NN | USER OAUTH ID: ${userOauthID}`
          )
        );
        networkId = await createUserNetwork({ userOauthID: userOauthID });
        nnDoc = await global.artyouDb.NeuralNetwork.findOne({
          id: networkId,
        }).populate("networkInput");
      }
      ...
      console.log(
        chalkAlert(`NNT | CREATING NN | USER OAUTH ID: ${userOauthID}`)
      );
      networkId = await createUserNetwork({ userOauthID: userOauthID });
      nnDoc = await global.artyouDb.NeuralNetwork.findOne({
        id: networkId,
