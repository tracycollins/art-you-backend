...
NeuralNetworkTools.prototype.createNetwork = async function (p) {
  try {
    ...
    networkObj.numInputs = params.inputsObj
      ? params.inputsObj.numInputs
...
NeuralNetworkTools.prototype.createInputSet = async function (p) {
  ...
  try {
    ...
    // const dbInputsDoc = await inputsDoc.save();
    console.log({ inputsDoc });
...
async function modelCursorStream(p) {
  console.log(p);
  const params = p || {};
  const numInputs = params.numInputs;
  const dataHandler = params.dataHandler;
  ...
  await cursor.eachAsync(async function (doc) {
    ...
    const result = await dataHandler({ doc: doc, numInputs: numInputs });
    results.push(result);
    numProcessed++;

    console.log(
      chalkLog(
        `${MODULE_ID_PREFIX} | [${numProcessed}] DOC | ${model} | ID: ${doc.id} | RATE: ${doc.rate} | ARTIST: ${doc.artwork.artist.name} | ARTWORK: ${doc.artwork.title}`
      )
    );
  });
...
NeuralNetworkTools.prototype.createTrainingSet = async function (p) {
  try {
    ...
    // const createDefaultOnUserNotFound =
    //   params.createDefaultOnUserNotFound || false;
    ...
    if (!userDoc) {
      ...
      // if (createDefaultOnUserNotFound) {
    ...
    const inputsObj = await global.artyouDb.NetworkInput.findOne({
      id: inputsId,
    }).lean();

    console.log(
      chalk.blue(
        `${MODULE_ID_PREFIX} | INPUTS | ID ${inputsObj.id} ${inputsObj.numInputs} INPUTS `
      )
    );

    const dataHandler = async (params) => {
      const { doc, numInputs } = params;
      const inputTags = doc.artwork.tags.map((tag) => tag.id);

      console.log(
        `${MODULE_ID_PREFIX} | RATE | ID ${doc.id} | RATE: ${doc.rate} | USER: ${doc.user.id} | ARTWORK: ${doc.artwork.id} | TAGS: ${inputTags} `
      );

      const datum = {};
      datum.hits = [];
      datum.misses = [];
      datum.input = Array.from({ length: numInputs }, () => 0);
      datum.input = inputsObj.inputs.map((inputTag) => {
        if (inputTags.includes(inputTag)) {
          console.log(
            `${MODULE_ID_PREFIX} | RATE | ID ${doc.id} | RATE: ${doc.rate} | HIT: ${inputTag} `
          );
      ...
      datum.output = doc.rate * normalizeFactor;
      return { datum: datum };
    };
...
NeuralNetworkTools.prototype.runNetworkTest = async function (p) {
  try {
    ...
    const testSet = _.slice(totalSet, totalSet);
    let resultsFit = {};
    ...
    if (totalSet.length > 0) {
      ...
      fitOptions.callbacks.onEpochEnd = (epoch, logs) => {
        ...
        if (epoch % 100 === 0) {
          console.log(
            `NNT | FIT | EPOCH ${epoch} | RATE ${epochRate.toFixed(
              1
            )} | ETC: ${Math.floor(timeToCompleteMS)} | ERROR: ${error}`
          );
        }
      };
    ...
    // const dataTestObj = {};
    // dataTestObj.datum = {};
    // dataTestObj.datum.input = Array.from(
    //   { length: reloadedNnObj.numInputs },
    //   () => Math.random()
    // );
    ...
    // const hitReducer = (numHits, inputValue) =>
    //   numHits + (inputValue > 0 ? 1 : 0);

    if (totalSet.length > 0) {
      for (const dataObj of testSet) {
        if (dataObj.datum.hits.length > 0)
          console.log(`INPUT HITS`, dataObj.datum.hits);
        // console.log(dataObj.datum.misses);

        // for (const input of dataObj.datum.input){
        //   console.log(`DATAOBJ | INPUT: ${input}`);
        // }
        // dataObj.inputHits = dataObj.datum.input.reduce(hitReducer);
