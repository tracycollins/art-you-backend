...
NeuralNetworkTools.prototype.createNetwork = async function (p) {
  try {
    ...
    networkObj.id =
      params.id ||
      `nn_tf_${process.pid}_${networkObj.numInputs}_${getTimeStamp()}`;
...
NeuralNetworkTools.prototype.createInputSet = async function (p) {
  ...
  try {
    console.log(
      chalk.blue(`${PF} | OAUTH ID: ${oauthID} | CREATING INPUT SET ...`)
    );
...
NeuralNetworkTools.prototype.createTrainingSet = async function (p) {
  try {
    ...
    console.log(
      chalk.blue(
        `${PF} | CREATE TRAINING SET | OAUTH ID: ${oauthID} | INPUTS ID: ${inputsId}`
      )
    );
...
NeuralNetworkTools.prototype.createUserNetwork = async function (p) {
  try {
    ...
    console.log(
      `${PF} | createUserNetwork | OAUTH ID: ${oauthID} | LOAD DB NN DOC | ${networkDoc.id} | oauthID: ${networkDoc.meta.user.oauthID}`
    );
    ...
    console.log(
      `${PF} | createUserNetwork | OAUTH ID: ${oauthID} | LOADED DB NN DOC | ID: ${loadedNnDoc.id} | INPUTS: ${loadedNnDoc.numInputs} | HL: ${loadedNnDoc.hiddenLayerSize}`
    );
    ...
    console.log(
      `${PF} | createUserNetwork | OAUTH ID: ${oauthID} | RELOADED DB NN DOC | ID: ${reloadedNnObj.id} | oauthID: ${reloadedNnObj.meta.user.oauthID} | INPUTS: ${reloadedNnObj.numInputs} | HL: ${reloadedNnObj.hiddenLayerSize}`
    );
    ...
    if (totalSet.length > 0) {
      for (const dataObj of testSet) {
        if (dataObj.datum.hits.length > 0)
          debug(
            `${PF} | createUserNetwork | OAUTH ID: ${oauthID} | INPUT HITS: ${dataObj.datum.hits.length}`
          );
