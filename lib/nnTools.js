...
const DEFAULT_NETWORK_FIT_EPOCHS = 100;
...
NeuralNetworkTools.prototype.delay = function (p) {
  const params = p || {};
        ...
        console.log(
          chalkLog(
            PF +
              ...
              " MS" +
              " | START: " +
              getTimeStamp(delayStartMoment) +
              " | END: " +
              getTimeStamp(delayEndMoment) +
              " | NOW: " +
              getTimeStamp(0)
          )
        );
...
NeuralNetworkTools.prototype.createDataObj = async (params) => {
  const { artworkDoc, inputsDoc, ratingDoc, normalizeFactor, verbose } = params;
  ...
  datum.output = ratingDoc ? ratingDoc.rate * normalizeFactor : null;
  return { datum: datum };
};
...
NeuralNetworkTools.prototype.fit = async function (params) {
  try {
    ...
    defaultOptions.epochs = DEFAULT_NETWORK_FIT_EPOCHS;
...
NeuralNetworkTools.prototype.activateNetwork = async function (params) {
  ...
  try {
    ...
    const error = params.dataObj.datum.output
      ? nnOut - params.dataObj.datum.output
      : null;
    console.log(
      chalkLog(
        `${PF} | NN PREDICT` +
          ` | ${nnId}` +
          ` | DATUM: ${
            params.dataObj.datum.output
              ? params.dataObj.datum.output.toFixed(5)
              : "UNDEF"
          }` +
          ` | NN OUT: ${nnOut.toFixed(5)}` +
          ` | ERROR: ${error ? error.toFixed(5) : "N/A"}`
      )
    );
...
NeuralNetworkTools.prototype.updateRecommendations = async function (p) {
  try {
    const params = p || {};
    const job = params.job || { id: "UNDEFINED" };
    ...
    console.log(chalk.blue(`${PF} | JOB NAME:      ${job.name}`));
    console.log(chalk.blue(`${PF} | JOB ID:        ${job.id}`));
...
NeuralNetworkTools.prototype.createUserNetwork = async function (p) {
  try {
    ...
    console.log(
      `${PF} | createUserNetwork | OAUTH ID: ${oauthID} | COMPILING TENSORFLOW NETWORK ...`
    );
    ...
    if (totalSet.length > 0) {
      ...
      fitOptions.callbacks.onEpochEnd = (epoch, logs) => {
        ...
        const epochsPerSec = elapsedInt > 0 ? (1000 * epoch) / elapsedInt : 0;
        ...
        if (epoch % 100 === 0) {
          console.log(
            `${PF} | createUserNetwork | FIT` +
              ` | OAUTH ID: ${oauthID}` +
              ` | EPOCH ${epoch}/${fitOptions.epochs}` +
              ` | RATE: ${epochRate.toFixed(1)} ms/epoch` +
              ` | RATE: ${epochsPerSec.toFixed(3)} epochs/sec` +
              ` | ELAPSED: ${msToTime(elapsedInt)}` +
              ` | ETC: ${msToTime(Math.floor(timeToCompleteMS))}` +
              ` | ETC: ${moment()
                .add(timeToCompleteMS)
