...
NeuralNetworkTools.prototype.createJson = async function(params){
  ...
  try{
    ...
    if (params.networkObj.networkTechnology === "tensorflow") {
      ...
      console.log(chalkLog(`${MODULE_ID_PREFIX} | ... CREATE TENSORFLOW JSON | NN ID: ${params.networkObj.id}`));
...
NeuralNetworkTools.prototype.convertNetwork = async function(params){
  ...
  try{
    ...
    if (empty(nnObj.networkJson)) {
      console.log(chalkError(`${MODULE_ID_PREFIX} | convertNetwork | *** NO JSON | TECH: ${nnObj.networkTechnology} | NN ID: ${nnObj.id}`));
      throw new Error("NO JSON NN");
    }

    console.log(chalkLog(`${MODULE_ID_PREFIX} | convertNetwork | TECH: ${nnObj.networkTechnology} | NNID: ${nnObj.id}`));
...
NeuralNetworkTools.prototype.activateNetwork = async function (params) {
  ...
  if (params.verbose) {
    console.log(chalkAlert(`TENSORFLOW | ${nnObj.id} | PREDICTION: ${prediction}`))
  }
...
NeuralNetworkTools.prototype.runNetworkTest = async function (p) {
  ...
  try{
    ...
    console.log(`NNT | CREATE DEFAULT INPUTS DOC | NUM INPUTS: ${configuration.numInputs}`)
    ...
    const loadedInputsDoc = await global.artyouDb.NetworkInput.findOne({id: testInputsDoc.id}).lean()

    console.log(`NNT | CREATE NN DOC | INPUTS: ${loadedInputsDoc.id}`)

    const testNetworkDoc = await createNetwork({ inputsObj: loadedInputsDoc })
    ...
    console.log(chalk.green(`NNT | FIT COMPLETE | NN ID: ${testNetworkDoc.id} | EPOCHS ${results.stats.params.epochs}`))
    console.log(chalk.green(`============================================================================================`))
    ...
    console.log(`NNT | LOAD DB NN DOC | ${testNetworkDoc.id}`)

    const loadedNnDoc = await global.artyouDb.NeuralNetwork.findOne({id: testNetworkDoc.id}).lean()

    console.log(`NNT | LOADED DB NN DOC | ID: ${loadedNnDoc.id} | INPUTS: ${loadedNnDoc.numInputs} | HL: ${loadedNnDoc.hiddenLayerSize}`)

    await loadNetwork({networkObj: loadedNnDoc})

    const reloadedNnObj = await convertNetwork({networkObj: loadedNnDoc})

    console.log(`NNT | RELOADED DB NN DOC | ID: ${reloadedNnObj.id} | INPUTS: ${reloadedNnObj.numInputs} | HL: ${reloadedNnObj.hiddenLayerSize}`)
