...
async function cursorDataHandler(params) {
  ...
  console.log(
    chalkLog(
      `${MODULE_ID_PREFIX} | cursorDataHandler | ARTWORK: ${artwork.title}`
    )
  );
  return;
}

async function artworkCursorStream(params) {
  const maxArtworkCount = 10;
...
NeuralNetworkTools.prototype.runNetworkTest = async function (p) {
  try {
    ...
    for (let i = 0; i < trainingSetSize; i++) {
      const datum = {};
      datum.input = Array.from({ length: testInputsDoc.numInputs }, () =>
        Math.random()
      );
    ...
    console.log(
      `NNT | RELOADED DB NN DOC | ID: ${reloadedNnObj.id} | INPUTS: ${reloadedNnObj.numInputs} | HL: ${reloadedNnObj.hiddenLayerSize}`
    );
