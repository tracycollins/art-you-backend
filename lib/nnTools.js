...
configuration.image = {};
configuration.image.defaultWidth = DEFAULT_IMAGE_WIDTH;
configuration.image.defaultHeight = DEFAULT_IMAGE_HEIGHT;
configuration.image.defaultChannels = 3;
configuration.fit = {};
...
NeuralNetworkTools.prototype.createNetwork = async function (p) {
  try {
    ...
    const imageInput = tensorflow.input({
      shape: [
        configuration.image.defaultWidth,
        configuration.image.defaultHeight,
        configuration.image.defaultChannels,
      ],
    });
    ...
    // const attributeNetwork = tensorflow.sequential();

    // attributeNetwork.add(
    ...
    // networkDoc.network.add(
    //   tensorflow.layers.concatenate([attributeNetwork, cnnNetwork])
    // );
...
NeuralNetworkTools.prototype.createDataObj = async (params) => {
  ...
  const image = inputsDoc.image
    ? inputsDoc.image
    : {
        width: configuration.image.defaultWidth,
        height: configuration.image.defaultHeight,
        channels: configuration.image.defaultChannels,
      };
