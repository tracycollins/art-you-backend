...
NeuralNetworkTools.prototype.runNetworkTest = async function (p) {
  ...
  try{
    ...
    console.log(`NNT | CREATE JSON | ${testNetworkObj.networkId}`)

    testNetworkObj.networkJson = await createJson({networkObj: testNetworkObj})

    delete testNetworkObj.network;

    console.log(`NNT | CREATE DB DOC | ${testNetworkObj.networkId}`)

    const nnDbObj = new global.artyouDb.NeuralNetwork(testNetworkObj)

    console.log(`NNT | SAVE DB NN DOC | ${nnDbObj.networkId}`)

    await nnDbObj.save();

    console.log(`NNT | LOAD DB NN DOC | ${nnDbObj.networkId}`)

    const loadedNnDoc = await global.artyouDb.NeuralNetwork.findOne({networkId: nnDbObj.networkId}).lean()

    console.log(`NNT | LOADED DB NN DOC | ID: ${loadedNnDoc.networkId} | INPUTS: ${loadedNnDoc.numInputs} | HL: ${nnDbObj.hiddenLayerSize}`)
