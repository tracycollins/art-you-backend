...
NeuralNetworkTools.prototype.createTrainingSet = async function (p) {
  try {
    ...
    const userObj = await global.artyouDb.User.findOne({
      oauthID: userOauthID,
    }).lean();

    console.log(
      chalk.blue(
        `${MODULE_ID_PREFIX} | USER | OAUTH ID ${userObj.oauthID} | NAME: ${userObj.name} `
      )
    );
    ...
    const dataHandler = async (params) => {
      ...
      console.log(
        `${MODULE_ID_PREFIX} | RATE | ID ${doc.id} | RATE: ${doc.rate} | ARTWORK: ${doc.artwork.id} | TAGS: ${inputTags} `
      );
...
NeuralNetworkTools.prototype.activateNetwork = async function (params) {
  ...
  if (params.verbose) {
    console.log(
      chalkAlert(`TENSORFLOW | ${nnObj.id} | PREDICTION: ${prediction}`)
    );
  }
...
NeuralNetworkTools.prototype.runNetworkTest = async function (p) {
  try {
    ...
    console.log(
      chalk.green(
        `NNT | FIT COMPLETE | NN ID: ${testNetworkDoc.id} | EPOCHS ${results.stats.params.epochs}`
      )
    );
    console.log(
      chalk.green(
